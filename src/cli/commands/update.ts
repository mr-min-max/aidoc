...
export const updateCommand = new Command("update")
  .description(
    "Update existing documentation based on code changes (diff-aware)",
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (!existingDoc) {
        spinner.fail(
          chalk.red(
            `File not found: ${options.target}. Run 'aidoc readme' first to generate it.`,
          ),
        );
        process.exit(1);
      }
      ...
      if (changedFiles.length === 0) {
        spinner.succeed(
          chalk.green("No code changes found. Documentation is up to date! ✅"),
        );
        return;
      }
      spinner.succeed(
        chalk.yellow(
          `Found ${changedFiles.length} changed files since ${sinceRef}`,
        ),
      );
      ...
      if (ctx.isMock) {
        updatedDoc = await ctx.generator.generateUpdate({
          existingDoc,
          changedFiles,
        } as any);
      } else {
        const diffSummary = await getDiff(sinceRef, "HEAD", ctx.cwd);
        const updateCtx = buildUpdateContext(
          existingDoc,
          changedFiles,
          diffSummary,
        );
